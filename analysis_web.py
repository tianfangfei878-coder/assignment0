...
if (typeof document !== "undefined") {
  ...
  function handle(f) {
    statusEl.textContent = "读取中: " + f.name;
    out.innerHTML = "";
    if (typeof Intl === "undefined" || !Intl.Segmenter) {
      statusEl.innerHTML = '<span class="err">当前浏览器不支持 Intl.Segmenter，请使用较新版本的 Chrome / Edge / Firefox / Safari</span>';
      return;
    }
      ...
      setTimeout(function () {
        ...
        setTimeout(function () {
          try {
            ...
            var ms = Date.now() - t0;
            statusEl.textContent = "完成，用时 " + ms + " ms";
            render(j);
          } catch (e) {
            statusEl.innerHTML = '<span class="err">分析失败: ' + e.message + "</span>";
          }
        }, 30);
      }, 30);
    };
    fr.onerror = function () {
      statusEl.innerHTML = '<span class="err">文件读取失败</span>';
    };
